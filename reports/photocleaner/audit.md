...
`develop` vs `main`: ahead 1, behind 3 (`gh api repos/ptr727/PhotoCleaner/compare/main...develop`). **Diverged, and the behind direction is the anomaly** - under the forward-only model `develop` never trails `main`, so the three `main`-only commits need a forward sync to `develop` (never a back-merge of `develop` into `main`).
...
| console | fail | pass | drift | a real System.CommandLine console app (`PhotoCleaner/PhotoCleaner.csproj:9 OutputType=Exe`, `:34 System.CommandLine 2.0.5`, net10.0) but no `build-executable-task.yml` and no release pipeline, so the smoke-subset and `release-asset-<branch>-<target>` checks have nothing to bind to |
