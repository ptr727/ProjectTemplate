...
| branch-model | drift | pass | drift | both branches protected (rulesets `develop`/`main` `enforcement: active`); live rulesets match `repo-config/{develop,main}.json` **except** the required-status-check `context` = `Check pull request workflow status job` vs canonical `Check pull request workflow status` - matches PlexCleaner's own aggregator `test-pull-request.yml:48`, so enforcement is intact |
...
- **D1.5 (one aggregator):** PASS - `check-workflow-status`, `needs: [validate, smoke-build]`, `always() && !github.event.deleted`, fails on any non-`success` (`test-pull-request.yml:47-63`). Name is ruleset-bound (with the `job` suffix - see branch-model drift).
...
3. **Ruleset required-status-check context carries a `job` suffix.** Live `develop`/`main` rulesets set the check `context` to `Check pull request workflow status job`; the fleet canonical (`repo-config/{develop,main}.json`) is `Check pull request workflow status`. Renamed the aggregator job (`test-pull-request.yml:48`) and both ruleset contexts in lockstep, so enforcement holds, but AGENTS.md/WORKFLOW.md advise against suffixing the ruleset-bound name. **(Recurs with Utilities.)**
