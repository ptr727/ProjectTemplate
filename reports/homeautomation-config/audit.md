...
Second full run, replacing the 2026-08-15 report in whole. That report graded the repository on the operational workflow model. The registry moved it to the release model on 2026-09-24 (hub #1766, promoted in #1768), the repository described itself that way in ptr727/HomeAutomation-Config#431 (promoted in #432), and it was resynced against the hub the same day in ptr727/HomeAutomation-Config#434, promoted to `main` in #435. No release was dispatched after the promotion, so the newest GitHub release predates the reclassification. This run grades `main` at that promotion merge, and it is the first run of this repository against the `release` rulesets, the release trigger shape, and the `parity.hooks` check, which landed on 2026-08-23 after the previous report.
...
| agent-instruction-set | pass | pass | pass | The mechanical run reports no verbatim-section, verbatim-tree, or template-reference finding after #434. `AGENTS.md` carries exactly the three declared sections (`AGENTS.md:9,31,76`), and `.github/copilot-instructions.md:35` carries the Disproved Claims heading with the canonical rules and no repository entries |
...
- **The four intent advisories are date-only.** `.editorconfig`, `.editorconfig-checker.json`, `.gitattributes`, and `version.json` are each byte-identical to the hub's current canonical at `2c5802f`, so there is nothing to carry. The advisory fires because the repository's last commit on each file predates the hub's last commit on the canonical, and an intent finding verified satisfied has no way to be cleared. That is hub #727, so these four advisories recur on every run until it lands and are not a repository finding.
...
- ptr727/HomeAutomation-Config#431 (merged into `develop` at `03e9c0e`) and #432 (merged to `main` at `70f223b`): the repository's own description of the release model, following the registry change.
