...
6. **Regenerate and commit all trees together**: `python3 scripts/build_dist.py`, then, once the user authorizes committing per `git-commit-conventions`, commit the source and both generated trees in one commit. CI runs `--check` on every pull request and fails a desynced distribution. `python3 scripts/tests/test_build_dist.py` covers the generator itself.
