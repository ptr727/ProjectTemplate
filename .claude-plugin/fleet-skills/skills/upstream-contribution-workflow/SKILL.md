...
5. **If upstream reviewers ask for changes, apply them to the dirty branch first**, iterate there
   ...
   round means rewriting the clean branch's history and pushing it again. This is expected and
   fine here: the clean branch is a single-purpose presentation branch on the maintainer's own
   fork, nobody else's work sits on top of it, which is exactly the shared-history condition the
   fleet's own never-force-push rule (`git-commit-conventions`, scoped to this fleet's own repos)
