...
2. A review is confirmed on the **current head SHA**, matched by commit SHA rather than assumed
   ...
   the PR outright, or say it read only part of the changed files. A coverage statement from the
   newest round that states one stands in for the head round's own only where the pull request
   changes the same set of files at both commits, since a statement about a diff this head no
   longer has says nothing about this one, and `pr_review.py` refuses the carry where it cannot
   read that set at both. The coverage this item
