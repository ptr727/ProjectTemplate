...
- Sweep the spaced-hyphen (`dash`) prose class out of the carried rule books, the way the semicolon class was swept. It is the same argument - the correct-as-you-edit policy has no downstream trigger for verbatim content - at roughly 350 instances across `GOVERNANCE.md`, `CODESTYLE.md`, `WORKFLOW.md`, and `AUDIT.md`, so it is its own change and wants its own review.
