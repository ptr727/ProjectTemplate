...
- [ ] Configure git for SSH signing **before making any commits** (the signed-commits ruleset rejects unsigned history, and retrofitting it forces a full re-sign - see [GitHub Setup](#template---github-setup)), plus SSH forwarding in dev containers - see [docs/host-setup.md](./docs/host-setup.md), [docs/ssh-signing.md](./docs/ssh-signing.md), and [docs/devcontainer.md](./docs/devcontainer.md).
...
- [ ] Configure [GitHub](#template---github-setup) for the new repository - including deleting any classic branch protection and creating the two `develop`/`main` rulesets by exporting/importing the template's (see [Rules / Rulesets](#template---github-setup)).
...
- Rules / Rulesets - **separate rulesets per branch**. Develop and main intentionally diverge on two rules - allowed merge methods and `Require linear history`. `Require branches to be up to date before merging` is **off on both** for related-but-distinct reasons (below); everything else is shared.
  - **Configure these by importing the committed ruleset JSON (`.github/rulesets/develop.json`, `.github/rulesets/main.json`) - do not hand-build the rules.** Those files are the versioned, PR-gated source of truth - they live only in this template and are imported/diffed against a repo's live config during porting and re-sync (see [AGENTS.md "Staying in Sync"](./AGENTS.md#staying-in-sync-and-reporting-drift-upstream)), never copied into derived repos; the result must be **exactly two rulesets named `develop` and `main`** (the names are load-bearing: `AGENTS.md` and these docs reference them). Reconstructing each rule by hand is the step that has gone wrong on past ports.
...
For each output you **don't** ship, delete its `build-<target>-task.yml`, its job + `needs` entry in `build-release-task.yml`, its `test-pull-request.yml` path filter, and (PyPI) the `publish-pypi` job. So `build-release-task.yml` is edited to reflect *your* set of leaf jobs, but its `github-release` logic stays as-is - that's the part you reuse rather than fork. `get-version-task.yml` installs the .NET SDK to run NBGV (which produces the version/tag) even in a non-.NET repo. See [`AGENTS.md`](./AGENTS.md) "Release Model" for the full seam contract and the no-op-republish guarantee.
...
- **Factor the `cleanup-artifacts` job into a reusable workflow** so the run-artifact-deletion housekeeping lives in one place instead of being copy-pasted into each entry-point workflow (currently duplicated in `test-pull-request.yml` and `publish-release.yml`). Trigger: a third workflow needs the same cleanup, or the duplicated copies drift. Ref [ptr727/ESPHome-NonRoot#82](https://github.com/ptr727/ESPHome-NonRoot/pull/82#discussion_r3471767650).
...
- **Generalize the input-validation step into a reusable pattern** applied wherever a reusable workflow consumes a free-form input that fails late and cryptically (e.g. a `repositories` string later fed to `fromJSON(...)` errors deep in matrix expansion, not at the call site). Validate-and-fail-early with a clear message — same shape as the `publish-docker-readme-task.yml` "Validate inputs step" — so callers get an actionable error. Trigger: the next reusable workflow with a parse-or-die input, or a derived repo that hits a hard-to-diagnose `fromJSON`/matrix failure. Ref [ptr727/NxWitness#448](https://github.com/ptr727/NxWitness/pull/448#discussion_r3471769663).
