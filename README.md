...
- When ready, *only when ready*, create `main` branch from `first-branch` with no history:
  ...
  > **Warning - sign from the very first commit.** The `Initial import (squashed)` commit below - and every commit after it - must be **cryptographically signed**. Configure SSH signing (the Prerequisites above) *before* running these commands, and verify it is live: `git config --get commit.gpgsign` is `true` and a signer is loaded (`ssh-add -L` for SSH). Do **not** enable the `Require signed commits` ruleset (below) until the branch's history is fully signed. Enabling it on a branch that already contains unsigned commits forces you to **rewrite the entire history and re-sign every commit under your own identity** - you cannot sign another contributor's commits for them, so original authorship is lost. Order: signing first -> clean signed history -> then the ruleset.
