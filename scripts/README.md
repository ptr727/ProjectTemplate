...
`wait` exits `40` when Copilot answers the request with a plain comment rather than a review, meaning a comment of its own that postdates its newest review on the pull request. That shape is the one a refusal takes, a quota or rate-limit message among them, and it satisfies no coverage check by design, so a wait that reads formal reviews alone reads a refusal as an unmet condition and then polls out its whole timeout against an answer that already arrived. The comment prints whole, since its wording is the only thing separating a refusal from an ordinary remark, and a refusal is terminal: no review follows it and re-requesting does not clear it, so `40` ends the wait and hands the text to the maintainer. A comment **older** than the newest review is spent rather than terminal, because the review it preceded did land. Both queries read the newest `COMMENT_WINDOW` comments rather than the reviewer's own, since GraphQL offers no author filter, so ordinary discussion is what pushes an answer out of reach and the window is wide enough that it takes a hundred comments to do it. A **full** window reports `answered_outside_review=unknown` rather than `no`, because that is the one case where finding nothing and having nothing to find are the same reading, and a case holds the guard's number equal to the one the queries carry so a drift between them cannot read clean. The timeout path prints the full digest for the same reason, as a bare `PENDING` line reports a slow reviewer and a broken poll identically, which is the reading that turns a stalled watcher into a watcher nobody notices is stalled.
