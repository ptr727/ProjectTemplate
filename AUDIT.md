...
**Every check below is judged by hand, and none of them is mechanized.** [`spec/audit.py`][audit-runner] evaluates no check in `spec/project-types.json`, and reads that file for one purpose only, to resolve the id a registry `driftNote` names (section 8) against the catalog and against the repo's declared types. Resolving an id is not running the check it names, so a clean run is evidence about the deterministic subset in section 0 and evidence about **nothing** in this section. Read a clean run that way, because the two are easy to conflate: adding a check here changes what an auditor must judge and changes no tool's output, so the check reports nothing until someone evaluates it, and silence from a tool that was never looking reads exactly like a pass. Cite the `file:line` each check was judged against, since that citation is the only durable record that the judgment happened.
