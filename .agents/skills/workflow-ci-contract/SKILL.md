...
**actionlint's own file list is `.github/workflows/*.yml` and `*.yaml`.** It opens a composite action's `action.yml` only through a workflow in that list carrying `uses: ./<path>`, wherever in the tree that path leads, and only to judge that caller's `with:` block against the action's declared inputs, reporting an undefined input and a missing required one against the calling workflow. It never validates the action file as action metadata, and pointing it at one directly makes it parse the file as a workflow and report several syntax-check errors, so widening its file list is not available. **Where no workflow in the repository names the action, actionlint reaches it not at all**, which is the ordinary shape for a repository whose hooks are invoked from a hub reusable workflow rather than from a workflow of its own.

**The action file's own metadata is checked against GitHub's action schema instead**, `check-jsonschema`'s `vendor.github-actions` builtin, run as `uvx check-jsonschema@latest --builtin-schema vendor.github-actions -- <files>`. That one reads every tracked `action.yml` and `action.yaml` under `.github/actions/`, whether or not a workflow references it, and reaches the structure and the keys rather than the caller's contract. An action file outside that tree keeps actionlint's caller check and gets no metadata check at all.
