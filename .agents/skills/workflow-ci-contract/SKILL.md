...
- **Applicability.** A guarantee, or a 5B scenario from `WORKFLOW.md` section 5 tracing one, governing a construct the repo does not contain is N/A: recorded, excluded from the verdict, never a defect. A source-only pipeline is mostly N/A and that is fine.
