...
Most of the evidence is in the workflow files and the composite actions they reach. A guarantee whose evidence lies outside them names where, and in practice that is the repo's branch ruleset, its Actions and Dependabot secret names, a workflow the repo only calls, a project or dependency file, or a committed file such as `version.json`, `.github/dependabot.yml`, `global.json`, `codecov.yml`, `.gitignore`, or `.editorconfig`.

Cite what each verdict rests on. That is `file:line` for a file in the audited repo, the setting's own name where a repository setting rather than a file is the evidence, and `<owner>/<repo>@<sha>` plus the `file:line` in that repo where the guarantee binds a workflow the audited repo only calls, read at the SHA the caller pins. An **N/A** verdict names the absent construct instead, there being no line to cite.
...
| S11 | scheduled upstream-version bump (wrapper) | resolver detects a change -> commits the state file -> opens a `<prefix>-<branch>` PR -> the merge-bot auto-merges it where the tracker's `bump-branch-prefix` and `branches` match a merge-bot rule, and a tracker setting `auto-merge: false` leaves it for the maintainer instead (D8.3) -> the `main` pin publishes via the gate (a develop pin does not auto-publish, shipping instead via a develop dispatch or promotion) | D8.3, D3.5 |
