...
- **Resolution** - The `agent-conduct` skill ships with the narrow decision-moment triggers the proposal specifies (about to claim done, about to assume, a failure or review finding just surfaced a lesson, and, added later, about to write a handoff, with filing a question as an issue instead of asking it a further trigger on that same rule rather than a moment of its own), carrying `Verification Discipline`, `Communicating with the User`, and `Durable Knowledge and Self-Improvement` whole as generated includes (as summaries, before the include mechanism landed), the sections keeping the full rules and the surfacing pointer, while the carried AGENTS.md sections stay the always-on layer.
...
- **Trigger** - **Narrow, at decision moments**: about to claim work is done without having verified it, about to proceed on an assumption the user could cheaply confirm, a failure or review finding just surfaced a durable lesson, or about to write a handoff, which owes an account of every question parked rather than asked. It also fires whenever a session is about to file a question as an issue instead of asking it, which is a trigger rather than a fifth moment, since the rule it reaches is already covered by a moment above. Deliberately not always-on: the carried [`AGENTS.md`][agents] sections are the always-on layer, and an always-on conduct skill would duplicate them and spend the tokens the delegation rules exist to save.
