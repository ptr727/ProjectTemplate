...
- **Trigger** - **Narrow, at decision moments**: about to claim work is done without having verified it, about to proceed on an assumption the user could cheaply confirm, a failure or review finding just surfaced a durable lesson, or about to write a handoff, which owes an account of every question parked rather than asked. It also fires whenever a session is about to file a question as an issue instead of asking it, which is a trigger rather than a fifth moment, since the rule it reaches is one of the four above. Deliberately not always-on: the carried [`AGENTS.md`][agents] sections are the always-on layer, and an always-on conduct skill would duplicate them and spend the tokens the delegation rules exist to save.
