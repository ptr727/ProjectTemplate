...
Quote each entry, and give each one a version. The `unit-test` job's entry step refuses an array holding an entry with no non-whitespace character, which is the value that would otherwise run green on an interpreter nobody chose: `setup-uv` trims what it reads and then reads an empty input as an absent one, and a caller reaches that by interpolating an unset value of its own, or by leaving a stray space beside it. It refuses an unquoted entry too, though that one announces itself either way, since `fromJSON` admits a JSON array of numbers and a decimal form that survives the round trip runs the right interpreter while one that loses a trailing zero, `3.10` becoming `3.1`, fails instead. That failure lands in the `Sync dependencies step` rather than in `setup-uv`, which exports the version rather than resolving it. What the check buys there is a named cause rather than a silent pass prevented. An empty array, and anything that is not a JSON array at all, never reach the step: they fail while the matrix is expanded, loudly and with an unnamed cause.
