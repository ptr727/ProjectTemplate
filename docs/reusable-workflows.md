...
**Release repos with a smoke build** carry the standard `pull_request` trigger, a `changes` paths-filter job (WORKFLOW.md D1.1: each of the repo's own targets gets an entry naming the paths it is built from, which leaves both docs and `.github/workflows/**` matching nothing and satisfies D1.4 without naming a workflow path at all), and a `smoke-build` job. The smoke build calls the repo's own `./.github/workflows/build-release-task.yml` by local path rather than a hub task, since that orchestrator is not hosted until [Stage 4][stage-4].
...
jobs:
  ...
  # One entry per target this repo builds, naming the paths that target is built from, so a touched target never falls through unfiltered (D1.1).
  # A change touching none of those paths marks nothing and the smoke build skips, which is what scenario S2 requires of a docs-only pull request and S3 of a workflow-only one.
  # Naming what must not build instead, as '!.github/workflows/**', marks a docs-only pull request as a target change and fails S2.
  changes:
