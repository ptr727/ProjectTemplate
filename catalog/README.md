...
- `snippets/husky/` and `snippets/pre-commit/`: the two local commit-hook shapes, Husky.Net for a repo with a host .NET toolchain and the `pre-commit` framework for a repo without one, both carrying any language's checks plus the same shared doc gates (prose/comment-style, line endings) via `snippets/hub-fetch-run.py`. A repo may instead wire an equivalent hook on `core.hooksPath`, which is what this repository's own `.husky/` does. See each directory's own README.md.
