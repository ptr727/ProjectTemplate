...
A caller stub for a hub-hosted task carries no snippet of its own until the task ships in a release: its `uses:` line would pin a commit no release carries yet, which the pin gate rejects. `test-pull-request.yml` and `run-periodic-codegen-pull-request.yml` gained theirs once `2.0.352` released `validate-task.yml` and `run-codegen-pull-request-task.yml`. `publish-release.yml` gained its own once a release carried the renamed .NET publish interface and the package push had moved to the caller's own job. [`docs/reusable-workflows.md`][reusable-workflows] "Adopting the Gates" and "Adopting the Type-Specific Tasks" carry the remaining stub shapes as reference until each has a snippet of its own. `get-version-task.yml` and `publish-plan-task.yml` below carry no snippet at all, for a different reason: each is called as a job inside a larger stub rather than reached by its own top-level caller.
