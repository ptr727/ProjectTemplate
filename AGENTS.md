...
- **Fixing a non-compliant file:** bring it to its `.editorconfig` ending as a **deliberate, isolated change** (its own commit, EOL-only), never mixed into a content edit - so the line-ending churn is reviewable on its own.
