...
Treat this file as authoritative for everything else; don't restate its rules elsewhere. A derived repo's **project-specific conventions and public-API/behavioral contracts** (e.g. a "Library API Conventions" section) also live here, **not** in [`.github/copilot-instructions.md`](./.github/copilot-instructions.md) - that file is read only by GitHub Copilot / VS Code, while this file is read by every coding agent, so any rule a reviewer must honor has to live here to be provider-independent.
