...
- **Honor [`.editorconfig`](./.editorconfig) for line endings on every file you write.** It sets **CRLF** for `.md`, `.cs`, XML/`.csproj`/`.props`/`.targets`, `.yml`/`.yaml`, `.json`, and `.cmd`/`.bat`/`.ps1`; **LF** for `.sh`. `.gitattributes` is `* -text`, so git stores the exact bytes you commit and will **not** normalize endings for you - a tool that rewrites a file in text mode (a script, a bulk find/replace) can silently flip CRLF to LF and turn a one-line change into a whole-file diff.
- When editing an existing file, **preserve its current line endings** rather than reflowing them. After any programmatic edit, verify before staging: `git diff --stat` should touch only the lines you changed (not every line), and `file <path>` should report the expected `CRLF`/`LF`. If a diff balloons to the whole file, you flipped the endings - restore them and re-stage.
