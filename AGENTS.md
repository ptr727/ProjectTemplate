...
- **Versioning is semantic and maintainer-controlled.** The `version` (major.minor) in [`version.json`](./version.json) is the **deliberate** version floor; NBGV appends the git height for the actual build version. **Bump it only for a real release** - new features or significant changes the maintainer judges worth a new version. **Routine maintenance does not bump it**: dependency bumps, CI/workflow fixes, doc edits, and template re-syncs land on `main` as a new *patch* of the current version, never a new minor. There is **no automatic versioning** - the file changes only when someone edits it.
  - **`develop` and `main` normally share the same `version.json`.** `develop` is the prerelease channel of the current version, `main` is the public release; NBGV computes the exact build version (`major.minor.height`, with a prerelease suffix off `main`) from `version.json` plus git height. **`develop` is not required to "lead" `main`**, and you should **not preemptively bump** "for the next cycle" - bump when you actually cut the feature release. (A premature develop-side bump is what forces the awkward "promote maintenance without the version" dance below; avoid creating it.)
  - **Promoting `develop -> main` carries `version.json` as-is.** If the maintainer bumped it, the promotion *is* the release of that version. If it's unchanged (maintenance/catch-up), `main` simply advances a patch - nothing special to do. **If `develop`'s version was bumped ahead of `main` but you want to promote only maintenance** (not release the new version yet), hold the version on the promotion branch: `git checkout main -- version.json` before opening the PR, so `main` keeps its current version. This is the exception, not the routine.
