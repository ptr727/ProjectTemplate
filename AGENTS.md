...
- **Scripts and extensionless executables must be LF - and pinned in `.gitattributes`, not just configured.** A CRLF shebang (`#!/usr/bin/env bash\r`) breaks execution. `.editorconfig` sets `[*.sh] = lf`, but it cannot match **extensionless** executables (s6 service scripts `run`/`up`/`finish`, git hooks), and `* -text` enforces nothing - so a broad normalization pass or an editor can silently flip them to CRLF (it has). `.gitattributes` is the enforcement layer: it carries `*.sh text eol=lf`, and a repo shipping extensionless scripts **adds an explicit path pin** (e.g. `Docker/s6-overlay/** text eol=lf`) so git holds them at LF on checkout and `--renormalize`. This pin is mandatory for any repo that overrides s6 init or ships executable scripts.
...
- **[`CODESTYLE.md`](./CODESTYLE.md)** - the single code-style guide. **Carry the whole file verbatim**, all language sections (.NET, Python) included - a section for a language the repo doesn't ship is inert and costs nothing, and keeping it makes re-sync a clean overwrite rather than a per-section merge. **Repo-root placement is load-bearing**: `AGENTS.md` links it as `./CODESTYLE.md` and `.github/copilot-instructions.md` as `../CODESTYLE.md`, so moving it breaks those links. Adapt the in-section repo-specific bits - the .NET project-folder list, the `InternalsVisibleTo` project names, and the VS Code task labels - to your repo.
