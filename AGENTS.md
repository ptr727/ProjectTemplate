...
- **Codegen output must be deterministic from its inputs, never per-run state.** The codegen workflow runs a matrix over `main` and `develop`, each leg independently generating against its own checkout and opening its own PR. For the two branches not to conflict on `develop -> main`, the generator's output must depend only on stable inputs - an external source that changes periodically, not per build. This template's `CodeGen/` demo reads a daily-stable value (the API Ninjas quote-of-the-day): both legs on the same day produce identical output, and the only divergence is the rare day-boundary case (the source updates between the two legs), which is expected and self-heals on the next run. This matches how the real downstream codegen consumers work (e.g. `NxWitness`, `LanguageTags`). A generator that embeds per-invocation state (timestamps, GUIDs, build IDs) would diverge every run and conflict on every release - don't do that. The demo's `--runtime` option (`now` -> `DateTime.UtcNow`, or an ISO-8601 literal) exists only to demonstrate that failure mode for a **local** run; CI never passes it.
