...
- **Versioning is semantic and maintainer-controlled.** The `version` (major.minor) in [`version.json`](./version.json) is the version floor; NBGV appends the git height for the build version. The maintainer edits it to cut a release - new features or significant changes worth a new version. Dependency bumps, CI/workflow fixes, doc edits, and template re-syncs leave `version.json` untouched and land on `main` as a *patch* of the current version.
  - **`develop` and `main` carry the same `version.json`.** `develop` is the prerelease channel of the current version; `main` is the public release. NBGV computes the build version (`major.minor.height`, with a prerelease suffix off `main`) from `version.json` plus git height.
  - **A `develop -> main` promotion carries `version.json` as-is.** A maintainer-bumped version releases that version; an unchanged version advances `main` by a patch. When `develop` and `main` hold different versions and a promotion should keep main's version, run `git checkout main -- version.json` on the promotion branch before opening the PR.
