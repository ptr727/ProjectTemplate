...
- **[`.github/rulesets/develop.json` and `.github/rulesets/main.json`](./.github/rulesets/)** - the versioned branch rulesets (squash + linear history on `develop`, merge on `main`; `strict_required_status_checks_policy: false` on **both**; everything else shared). Copy **verbatim** - they are repo-agnostic (`conditions` key on `refs/heads/{develop,main}`, `bypass_actors` uses the global Admin role `actor_id: 5`, the required check binds by name), so there are no placeholders to change. These differ from every other carried file in one way: carrying the file only keeps the *source of truth* in sync - it changes a repo's **live** GitHub rulesets only when the JSON is **imported** (`POST` at setup, see [README "Rules / Rulesets"](./README.md#rules--rulesets)) or **PUT** (drift correction, see [Staying in Sync](#staying-in-sync-and-reporting-drift-upstream)).
