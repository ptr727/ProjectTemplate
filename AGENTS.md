...
- **Artifact retention**: intermediate build artifacts (`actions/upload-artifact`) are an intra-run handoff only - their durable copies live on the GitHub release, not in workflow artifacts - so they must not survive the run and accumulate against the small account-wide artifact-storage quota. `publish-release.yml`'s terminal `cleanup-artifacts` job (`needs` every producer/consumer; gated to real publishes but with `always()` so a failed publish still cleans up; `permissions: actions: write`) deletes the run's artifacts via the REST API once every consumer has read them, tolerating individual delete failures so housekeeping never reds a successful publish. Set `retention-days: 1` on the uploads as a backstop; only `publish-release` runs persist artifacts (PR smoke builds gate every upload on `!smoke`).
