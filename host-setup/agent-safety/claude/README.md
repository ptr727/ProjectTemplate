...
- **Containment wraps hook commands too.** Claude Code applies `CLAUDE_CODE_SHELL_PREFIX` to hook commands as well as to Bash tool calls, so the guard and the sweep each run inside a `claude-hook-` scope. A hook scope is bounded and never stopped at session end, since a sibling SessionEnd hook may still be running inside one. Each wrapped command costs about 35 ms on the reference host, measured as two Python starts and one scope creation.
