...
- **Hook commands pass through the prefix unscoped.** Claude Code applies `CLAUDE_CODE_SHELL_PREFIX` to hook commands as well as to Bash tool calls. The prefix runs a hook under its shell directly, because a PreToolUse hook exiting 1 is a non-blocking error, so a scope that failed to start would let the command the guard denies go ahead. A hook is told apart by the `CLAUDE_PROJECT_DIR` Claude Code sets for hooks and the shell snapshot a tool call sources first, and either sign of a tool call contains it. Where a tool call's scope fails to start, that call fails with systemd's own error rather than running uncontained. Each contained tool call costs about 35 ms on the reference host, measured as two Python starts and one scope creation, and a hook command about 10 ms.
