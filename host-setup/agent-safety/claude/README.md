...
grep -c CLAUDE_CODE_SHELL_PREFIX ~/.claude/settings.json      # expect 1 where a user manager runs, 0 elsewhere
...
Every other key in the file is left as it stands, `permissions.allow` and the rest of `env` included, apart from the rules whose prefix this kit owns. The `env` entry is written only on a host with a running systemd user manager.
...
- **Containment runs on Linux and WSL with a systemd user manager and a `python3` 3.11 or newer on PATH.** The prefix runs through its `env python3` shebang for every hook command, the guard included, because Claude Code quotes the whole prefix value as one word and so no interpreter can be named in front of it. A PreToolUse hook that fails is a non-blocking error, so a `python3` unable to run the prefix would fail the guard open, and the installer checks that interpreter before registering anything. macOS and Windows have no systemd, and a `ulimit -u` fallback caps every process the user owns rather than one command's tree, so it would fail unrelated work rather than stop a runaway. The installer leaves the prefix unset there, and `--report` flags a prefix registered on a host that can no longer contain. Where the prefix is registered and the user manager later becomes unreachable, each command runs unbounded and prints one `tool-containment:` line on stderr saying so.
