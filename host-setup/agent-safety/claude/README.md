...
- **The sweep is POSIX only.** Whether a descendant outlives the agent is decided by comparing session ids. `ps` reports those and Windows has no equivalent of them. The installer deploys and registers the sweep on every platform, and the script returns immediately on a non-POSIX one. A Windows host therefore carries a hook that reports nothing, rather than an untested guess at the same question. That keeps the installer, the payload digest, and the registration checks identical everywhere, which is worth more than a Windows branch nobody has run.
