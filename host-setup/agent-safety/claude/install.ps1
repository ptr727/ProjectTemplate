...
# The installer and the hook use Python 3 syntax, so a bare `python`, which is Python 2 on some systems, is the last resort.
if (Get-Command "py" -ErrorAction SilentlyContinue) {
    & py -3 $script @args
} elseif (Get-Command "python3" -ErrorAction SilentlyContinue) {
    & python3 $script @args
} elseif (Get-Command "python" -ErrorAction SilentlyContinue) {
    # Verify a bare `python` is Python 3 before handing it Python 3 syntax, since it is Python 2 on some setups and would fail to parse install.py.
