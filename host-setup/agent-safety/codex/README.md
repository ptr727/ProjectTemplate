...
leaves behind, so it belongs wherever Codex signals that a session has ended, and
[`../claude/stray-process-sweep.py`][claude-sweep] is the worked reference for that one. Requirement 9
wraps every command rather than judging one, so it belongs wherever Codex lets a command be run
through a wrapper, with [`../claude/tool-containment.py`][claude-contain] as its reference. If Codex
offers none of these extension points, that finding belongs on
