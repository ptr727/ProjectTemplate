...
- [`menu.ps1`][menu-ps1] is the same front end for native Windows, over `host-setup\windows\` instead. It runs under Windows PowerShell 5.1 and hands off to PowerShell 7 the same way `bootstrap.ps1` does, and it adds one menu entry with no Linux peer, reporting the WSL platform through `setup-wsl.ps1`.
...
- [`agent-safety/`][agent-safety] holds the host-safety guards, deployed per machine and per account.
...
It answers a different question than `bootstrap.sh`/`bootstrap.ps1` do. Those stand a host up and stop. The menu loops, because a person sitting down with it usually wants more than one thing done in a sitting, and it tells the hub apart from whichever repo it happens to be run from: a checkout of `ptr727/ProjectTemplate` gets the hub tasks (audit a cataloged repo, check the generated Skills distributions), a checkout of any other repo gets the downstream tasks too (check or pull the hub's verbatim-owned files into that repo's own worktree, and the same pair again for the verbatim rule sections inside a file that repo also owns the rest of, per [`scripts/carry.py`][carry]), and every run gets the host tasks this directory's `linux/` or `windows/` tooling already provides. Only the downstream tasks need a repo to run from. The hub tasks fetch the hub themselves when there is no local checkout to reuse, so they still show and still work when the menu is run entirely standalone, off no checkout at all.
...
A run that installs the skills, `--host`, `--dev`, or `--skills`, keeps its tree, because the Claude Code plugin it registers loads that directory in place rather than copying it. Removing the tree would leave the plugin registered against a directory that no longer exists. The kept tree sits at `skills-tree` under `${XDG_DATA_HOME:-~/.local/share}/host-setup` on Linux and under `%LOCALAPPDATA%\host-setup` on Windows, or under `--dir` where one is given. It carries the commit the run resolved, which is what `skills_install.py --report` names for it, since a tarball has no git to ask. The next such run extracts beside it and swaps the new tree in immediately before the skills step, moving the old one aside first and removing it last. A run that fails before that step, a failed download or a failed tools step alike, therefore leaves the plugin loading what it loaded before. Every other action, and a dry run of these three, extracts a transient tree under another name, which it removes on exit unless `--keep` is given. The installer replaces a registration naming a directory that is gone. Run from a kept tree, it also replaces one naming another tree the bootstrap extracted. It leaves every other registration alone, since a checkout registered by hand is somebody's choice, and it restores the one it removed where the new registration then fails.

The kept tree is never a git checkout, which is what keeps the loader's two properties: it needs no git, and it never modifies a checkout that belongs to somebody else.

## Three Rules This Directory Follows

**Group by whichever axis has one member.** `linux/` is three concerns on one platform, so it is a platform directory. Windows host tooling therefore sits at `windows/` rather than beside the Linux scripts, because the `winget` equivalent of `install-tools.sh` is a different program rather than a translation of one. It carries one registry record per tool where the Linux script carries four functions, since every Windows source is `winget` and the per-tool variation those functions exist for does not arise. `windows/` also carries a fourth script with no Linux peer, because WSL is a Windows-side concern. The loader and the menu are one concern, two platforms, so `bootstrap.ps1` sits beside `bootstrap.sh`, and `menu.ps1` beside `menu.sh`, at the top level rather than inside `windows/`. `agent-safety/` follows the same platform-grouping shape one level down: it holds one concern (host-safety guards) implemented per *agent* rather than per OS, so `claude/`, `codex/`, and `opencode/` sit beside each other the way `linux/` and `windows/` do, each holding that one agent's own files (a Claude Code implementation carries its own `install.sh`/`install.ps1` pair inside `claude/`, the same platform variation `linux/`/`windows/` carry at the top level, just nested one directory deeper because the outer axis here is the agent, not the OS). Its root `README.md` is the agent-agnostic spec those per-agent directories each implement, not a concern directory of its own.
