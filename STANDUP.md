...
**Re-run it with `--repo` once section 2 has carried the file**, because a bare run reads no declaration but the one at its own working directory, so any floor the target adds goes unapplied. No warning covers the gap here, in either direction: the gate names an ancestor's overlay only where that overlay declares a tool, so the stub section 2 carries draws none, and a target that does not exist yet cannot be named at all. So this re-run is the only thing that counts the target's floors:
