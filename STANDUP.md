...
**Re-run it with `--repo` once section 2 has carried the file**, because a bare run reads no declaration but the one at its own working directory, so any floor the target adds goes unapplied. No warning covers the gap here, and the reason is that the run above is not inside the target: it starts in the hub checkout, whose own root carries a declaration, and the gate looks for an unapplied overlay above it only where its working directory carries none. So this re-run is the only thing that counts the target's floors:
