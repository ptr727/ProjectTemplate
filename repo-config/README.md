...
The link is read and written through GraphQL rather than the REST endpoints the other groups use, so both modes need project access on the token beside the admin the ruleset endpoints require. A token that cannot read projects stops `apply` in its pre-flight with nothing written, and reads as a failing project group under `check`. A token that can read them but not write one reaches the link itself, the last of the five groups. Each command's own error names the scope to grant.
