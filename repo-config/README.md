...
`blocked` is a marker too, and unlike `decision` it never stands alone. The issue carrying it records work this repository is waiting to do, so it carries its own triage label as well, and what the marker adds is that the work waits on another issue, which the body names in another repository. Every count of the open backlog includes it, which is the opposite of what `handoff` below does and is why the two are stated separately. What a round then does with it, and what it withholds until the blocker clears, is the `backlog-burndown` Skill's "Ranking". When it goes on and when it comes off are set by [GOVERNANCE.md "Durable Knowledge and Self-Improvement"][governance-durable-knowledge], which is also what reads it back, so nothing here decides either.

- **`blocked`**: blocked on another issue, one in another repository or a `decision` issue here for a parked handoff.

A handoff parked by an unattended run carries `blocked` as the one exception to each of those three statements. It carries no triage label, since no handoff does. Its parking comment rather than its body names the `decision` issue in this repository it waits on. And it stays out of every backlog count, as every handoff does. When it goes on and comes off is set by [GOVERNANCE.md "Durable Knowledge and Self-Improvement"][governance-durable-knowledge] as well.
