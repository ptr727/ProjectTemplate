...
def render_report(spreads, promote, gaps, ledger):
    ...
    # Untriaged auto-detection is verbatim-only by design. An intent unit is judged by meaning, so a byte
    # diff is expected, not an anomaly - flagging every intent divergence would bury the report under the
    # normal spread (AGENTS.md alone differs in most repos). Intent divergences are tracked only when a human
    # records a disposition (e.g. .editorconfig-checker.json accepted), which still renders in the burn-down.
    # A verbatim hand-modification with no disposition is a genuine anomaly, so it is the one that surfaces.
    untriaged_files = []
    for e, sp in spreads:
    ...
    w("")
    w("Generated by `python3 spec/fidelity_honesty.py --report` - do not hand-edit. Curate dispositions in")
    w("[`spec/divergences.json`][ledger] and regenerate. Each row reflects a repo's ground-truth branch at")
    w("generation time. Git dates this file.")
    w("")
    ...
    w("")
    w("A past hub revision, not the current canonical - the audit already flags these as DRIFT. Copy the")
    w("current file down. No judgment needed.")
    w("")
    ...
    # Reference-link definitions live at the bottom of the document (AGENTS.md Markdown convention).
