...
The set spans two files. [`AGENTS.md`](../AGENTS.md) is the entry point every agent reads first, and carries only the rules that apply to every task (context and delegation) plus the map to the rest. [`GOVERNANCE.md`](../GOVERNANCE.md) carries the topical rule text, one section per topic. The split exists so an agent loads the section a task needs instead of the whole rule book, and it changes nothing about how a section is classified or checked.
