...
- **Build** is repo-owned in shape: the leaf a target runs. A repo owning its release task hosts its leaves itself. A repo calling the hub-hosted task shapes the .NET, NuGet, and PyPI leaves through a hook at `.github/actions/<job>/action.yml`, named for the build job (`dotnet-publish`, `build-nuget`, `build-pypi`), which the task runs in place of its hub default where that file exists.
