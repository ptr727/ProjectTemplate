...
- **D4.5** A failed build blocks every publish target: `github-release` needs every build, the terminal registry pusher (Docker) needs every other build and guards `!failure() && !cancelled()` so a disabled or unchanged target, skipped rather than failed, still lets it push, and a package target's separate `publish-<target>` job `needs:` the release-task call, so no build failure ships anything partial. A failed registry **push** is outside that: it runs after `github-release`, so it can leave a release and tag for a version the registry never received, recovered by re-dispatching rather than by cleanup.
