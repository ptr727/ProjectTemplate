...
For each *applicable* scenario, evaluate every job's `if:`/`needs:` against the inputs and emit the predicted **run/skip + version + release + artifact-end-state** table, then compare to the expected. A scenario governing a construct the repo does not contain is N/A, per `WORKFLOW.md` section 1, and an absent trigger is such a construct. A scenario governing a construct the repo does not contain is N/A, per `WORKFLOW.md` section 1, and an absent trigger is such a construct: read the publisher's own `on:` block, which is what actually decides where a path can fire. A publisher carrying only `workflow_dispatch` records S5, S6 and S9 N/A, their push and schedule paths never firing there, and a repo with no publisher records S5 to S10 N/A together. A whole scenario is dropped only where the repo lacks its construct outright, so one whose input the repo can reach stays applicable, with any unreachable arm of its expected output recorded N/A inside the row rather than the row dropped. Where a scenario's path runs through a workflow or composite action the repo only **calls**, trace that callee as the repo reaches it, read at the SHA the caller pins rather than at the callee's current default branch, which is the same evidence rule 5A states. Predicting from the callee's `main` predicts a table for YAML the audited repo never runs. Minimum set:
