...
def check_sha_pin(root: Path, files: list[str]) -> list[str]:
    ...
    A local ref (`./`, or a bare `.github/` path) or a self-repository ref (`$/`) names no ref to
    pin and is skipped. References under another owner are shape-checked but not resolved.
