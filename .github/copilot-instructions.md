...
the generator and never in the copy. Post no review comment on a file under those trees. Where the
file the fix belongs in is in the head tree, comment on that file instead, and otherwise state the
finding in the review summary.
