...
marker lines `<!-- include: <path> > <heading> -->` and `<!-- /include -->` that a copy carries
exactly as its source does, is fixed in the hub under the heading that key names, since the key
resolves against the hub's root rather than this repository's copy of the same file. Post no
review comment on a file under those trees. When the
pull request changes the file the fix belongs in, comment on that file instead, and otherwise state
