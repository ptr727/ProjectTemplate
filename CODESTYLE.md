...
- **`shellcheck` clean, and a deliberate exception carries its reason inline.** A `# shellcheck disable=SCxxxx` names why the rule does not apply here, so the next reader can tell a considered exception from an unread warning. [`repo-config/configure.sh`][repo-config-configure] is the worked example, carrying five `SC2016` disables where a single-quoted `jq` program must stay unexpanded, each with its reason on the same line.
...
[readme]: ./README.md
[repo-config-configure]: ./repo-config/configure.sh
[root]: ./.editorconfig
