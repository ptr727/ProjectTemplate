...
- **Bring each repo's `3rd Party Tools` entries onto the shared catalog.** Measured on 2026-08-08: 55 findings across four repos, every one of them a link or a description that disagrees with [`spec/third-party-tools.json`][third-party-tools].
  - **Blocked by** - Nothing.
  - **Issue** - None filed.
  - **Checked** - Every repo's default branch on 2026-08-08, with the hub read at its own `develop`, which now conforms.
  - **Settled** - The bulk is absent descriptions rather than wrong ones: 48 of the 55 are a tool listed with no description at all, across LanguageTags, MediaTools and PlexCleaner, and PlexCleaner alone accounts for 27. Of the remaining seven, three describe a tool differently from the catalog and four link it differently.
