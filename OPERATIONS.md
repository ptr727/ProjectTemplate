...
CI passes explicit `--check` lists, and a bare `python3 scripts/prose_lint.py [file]` runs `DEFAULT_RULES`, which is those two lists plus `comment-added`. What differs is the exit code rather than the coverage. The block below is what a local run does, and it is not what CI gates with. CI's only whole-tree run is [`.github/actions/validate`](./.github/actions/validate/action.yml)'s `--check charset-unknown --summary`, which is `continue-on-error`, so it reports and gates nothing. What gates a pull request is the `prose-gate` composite action, which passes no `--check` and so gates every rule in `DEFAULT_RULES`, `charset-unknown` included, scoped to the lines the pull request touches. A bare run here exits non-zero on any of the ten the block below names. `comment-added` is the eleventh, and it reads what a change writes rather than what a file holds, so a run with no `--diff` stands it down and says so on stderr, which is why no list below names it. Its two overrides are `PROSE_ALLOW_COMMENTS`, non-empty in the environment, which the gate reads directly and so reaches every hook that runs it, and the `comments` label on the pull request, which is what CI reads. The variable stands the rule down for as long as it is set rather than for one commit, and the composite action clears it so a runner's environment cannot decide a pull request. `sentence-split` and `sentence-length` are in neither, so nothing below runs them and a local run reaches them only by naming them. Run the CI invocations:
