...
Both modes need admin on the repository and project access on the token, the second because the fleet project link is read and written through GraphQL. A token that cannot read projects stops `apply` in its pre-flight with nothing written, and reads as a failing project group under `check`. A token that can read them but not write one reaches the link itself, the last of the five groups. Each command's own error names the scope to grant.
