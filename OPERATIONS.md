...
Two `gh` limitations on the current host, both worked around rather than fixed. **Both were observed on a distribution-packaged `gh` 2.46.0, and the GitHub CLI maintainers name that exact range, `2.45.x` and `2.46.x`, as broken by deprecated GitHub APIs**, which is the class both belong to. Neither has been re-tested against a `gh` installed from the official repository, so read them as limitations of that package rather than as permanent behavior, and re-test both before carrying either workaround further. [docs/host-setup.md](./docs/host-setup.md) states where `gh` must come from, and `scripts/host_gate.py` fails a host below the floor.
