...
- **Version** — [Nerdbank.GitVersioning](https://github.com/dotnet/Nerdbank.GitVersioning) (NBGV) shared with the .NET side. CI overwrites `_version.py` with NBGV's `AssemblyFileVersion` (`Major.Minor.Patch.BuildNumber`, PEP 440 valid) before `uv build`; that matches the .NET assemblies' `FileVersion` / `AssemblyVersion` stamp. NuGet packages and Docker tags use NBGV's `SemVer2` instead, so strings aren't byte-identical, but all four come from the same NBGV computation per release commit.
...
- **Pick a versioning scheme.** The template defaults to **NBGV-driven** versioning shared with the .NET side: `_version.py` holds `__version__ = "0.0.0"` as a local-development placeholder, and the CI step **"Write version into _version.py step"** in [`build-pypilibrary-task.yml`](../.github/workflows/build-pypilibrary-task.yml) overwrites it with NBGV's `AssemblyFileVersion` (always `Major.Minor.Patch.BuildNumber`, all numeric, PEP 440 valid) just before `uv build`. PyPI therefore ships the same version string that's stamped into the .NET assemblies as `FileVersion` / `AssemblyVersion`. The **NuGet package version** and **Docker tags** in this repo use NBGV's `SemVer2` instead (PEP 440 doesn't accept `SemVer2`'s prerelease / build-metadata suffixes), so those strings are not byte-identical to PyPI's — but all four artifacts derive from the same NBGV computation against `version.json` + git history and correspond to the same release commit. If you want a different scheme, replace both `_version.py` and the workflow step. Two common alternatives:
